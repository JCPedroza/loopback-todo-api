...
export class TodoListTodoListImageController {
  ...
  @get('/todo-lists/{id}/todo-list-images', {
    responses: {
      '200': {
        description: 'Array of TodoList has many TodoListImage',
        content: {
          'application/json': {
            schema: {type: 'array', items: getModelSchemaRef(TodoListImage)},
          },
        },
      },
    },
  ...
  async find(
    ...
    return this.todoListRepository.image(id).find(filter);
  }

  @post('/todo-lists/{id}/todo-list-images', {
    responses: {
      '200': {
  ...
  @patch('/todo-lists/{id}/todo-list-images', {
    responses: {
      '200': {
  ...
  @del('/todo-lists/{id}/todo-list-images', {
    responses: {
      '200': {
